...
class AutoBaselineModel():
    ...
    def __init__(self, expt, baselineCutoff=100):
        ...
        self.Q = 3
        self.A = normalize_rows([[99, 2, 5],    # signal
                                 [50, 50, 0],   # blink
                                 [0, 0, 1]])    # bleach
    ...
    def _calc_emission_prob(self):
        ...
        p_X = np.sqrt(2*np.pi*hcol(self.var))**-1 * np.exp(-0.5 * (Y-hcol(self.mu))**2 / hcol(self.var)) # K x T x M
        ...
        for itr in range(maxIter):
            ...
            for i in range(self.Q):
                for j in range(self.Q):
                    self.A[i,j] = np.sum(np.logical_and(sp[:, :-1]==i, sp[:, 1:]==j))/np.sum(sp[:, :-1]==i)
